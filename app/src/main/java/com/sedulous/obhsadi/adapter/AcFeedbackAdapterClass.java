...
public class AcFeedbackAdapterClass extends RecyclerView.Adapter {
    ...
    public AcFeedbackAdapterClass(Context context,JSONArray list,RatingSelectionInterface ratingSelectionInterface,boolean isEnglish)
    {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        try {
            ...
            try {
                if (jsonObject.getInt("sections") == 1) {
                    mHolder.tvIndex.setText("B" + (position + 1) + "");
                    mHolder.tvIndex.setTextColor(Color.BLUE);
                    mHolder.tvIndexText.setTextColor(Color.BLUE);
                } else {
                    mHolder.tvIndex.setText("A" + (position + 1) + "");
                }
            }
            catch(Exception e){
